...
//-->Codigo TP6.14
void imprBombardero614(coordenadas a[], int m,grafico g);
void imprTanque614(int x, int y,grafico g);
void cargarGraficos(grafico *g);
void liberar(grafico *g);
//<--
...
int main() {
	...
	while(true)
	{
		...
		//-->Codigo TP6.14
		imprTanque614(x,y,graficos); //imprime tanque
		//<--
		
		imprBomba(bo,MaxBomb); //imprime bomba 1
		
		//-->Codigo TP6.14
		imprBombardero614(av,MaxAvio,graficos); //imprimimos los aviones
		//<--
...
void liberar(grafico *g)
{
	...
	delete g->bombardero[0];
	delete g->bombardero[1];
}
...
void cargarGraficos(grafico *g)
{
	char aux[255];
	fstream file;
	file.open("tanque.txt");
	
	file.getline(aux,255);
	g->tanque[0] = new char[strlen(aux)+1];
	strcpy(g->tanque[0],aux);
	g->tanque[0][strlen(aux)] = '\0';
				
	file.getline(aux,255);
	g->tanque[1] = new char[strlen(aux)+1];
	strcpy(g->tanque[1],aux);	
	g->tanque[1][strlen(aux)] = '\0';
	
	file.getline(aux,255);
	g->tanque[2] = new char[strlen(aux)+1];
	strcpy(g->tanque[2],aux);
	g->tanque[2][strlen(aux)] = '\0';
				
	file.close();
	
	file.open("bombardero.txt");
	
	file.getline(aux,255);
	g->bombardero[0] = new char[strlen(aux)+1];
	strcpy(g->bombardero[0],aux);
	g->bombardero[0][strlen(aux)] = '\0';
		
	file.getline(aux,255);
	g->bombardero[1] = new char[strlen(aux)+1];
	strcpy(g->bombardero[1],aux);
	g->bombardero[1][strlen(aux)] = '\0';
	
	file.close();
}

void imprBombardero614(coordenadas a[], int m,grafico g) //dibuja los aviones en las coordenadas (x[n], y[n])
{
	for(int n=0;n<m;++n)
	{
		if((a[n].x<2)||(a[n].y<1)) continue; // si las coordenadas no son imprimibles no se imprime
		
		cout<<setxy(a[n].x-1,a[n].y)<<g.bombardero[0];
		cout<<setxy(a[n].x-1,a[n].y+1)<<g.bombardero[1];
	}
}

void imprTanque614(int x, int y,grafico g) //dibuja el tanque
{
	cout<<setxy(x-1,y)<<g.tanque[0];
	cout<<setxy(x-1,y+1)<<g.tanque[1];
	cout<<setxy(x-1,y+2)<<g.tanque[2];		
}
